...
public class ProxyUtils {
    public static void setDouble(double[] arr, JsonNode parentNode, String key, int index) {
        JsonNode listNode = parentNode.get(key);
        if (listNode != null && listNode.isArray() && index < listNode.size()) {
            arr[index] = listNode.get(index).asDouble();
        }
    }

    public static void setLong(long[] arr, JsonNode parentNode, String key, int index) {
        JsonNode listNode = parentNode.get(key);
        if (listNode != null && listNode.isArray() && index < listNode.size()) {
            arr[index] = listNode.get(index).asLong();
        }
    }

    public static void setDouble(double[] arr, JsonNode parentNode, String key, int index, int multiply) {
        JsonNode listNode = parentNode.get(key);
        if (listNode != null && listNode.isArray() && index < listNode.size()) {
            arr[index] = listNode.get(index).asDouble() * multiply;
        }
    }

    public static void setLong(long[] arr, JsonNode parentNode, String key, int index, int multiply) {
        JsonNode listNode = parentNode.get(key);
        if (listNode != null && listNode.isArray() && index < listNode.size()) {
            arr[index] = listNode.get(index).asLong() * multiply;
        }
    }
    ...
    public static JsonNode array2node(double[][] array, int cores, int num) {
        ...
        for (int j = 0; j < cache.length; j++) {
            node.add(String.format("%.4f", cache[j] / cores));
        }
        return node;
    }
    ...
    public static JsonNode array2node(double[] array) {
        ...
        for (int j = 0; j < array.length; j++) {
            node.add(String.format("%.4f", array[j]));
        }
        return node;
    }
    ...
    public static JsonNode array2node(long[] array1, long[] array2) {
        ...
        for (int j = 0; j < array2.length; j++) {
            if (array2[j] <= 0) {
                node.add(String.format("%.4f", 0.0));
            } else {
                node.add(String.format("%.4f", (double) (array2[j] - array1[j]) / array2[j]));
            }
        }
        return node;
    }
    ...
    public static JsonNode array2node(long[][] array1, long[][] array2, int num) {
        ...
        for (int j = 0; j < 6; j++) {
            if (cache2[j] <= 0) {
                ...
                node.add(String.format("%.4f", (double) (cache2[j] - cache1[j]) / cache2[j]));
            }
        }
